...
Merr (Meta-ERRor) is a syntax error message generator for LR parsers. It passes
...
L<Git repository|https://github.com/pippijn/merr>
...
C<sed -e 's/^\(%token[^"]*\w\+\)\s*".*"$/\1/'> will safely remove the string
literals as well as leading whitespace. This can be done in a C<make> target
like C<%.mly: %.mly.in>.
...
C<(state, token)> to its standard output. The C<state> should be a number, and
C<token> is the token type constructor name, e.g. C<TkIF>.
...
One or more C<open> directives are always required, and the opened modules
should provide the function C<< val string_of_token : token -> string >> and
...
C<open> and C<module> directives

In error messages, the C<%s> part of the message is replaced by the application
of C<string_of_token> with the erroneous token. So, for instance if that
function returns the C<string> argument of the C<TkIDENTIFIER> constructor, the
...
description parseable by merr, using the option C<-dump>.


=head3 Using the function
...
modified menhir parser will raise an exception C<StateError of int * token>
where C<token> is the token type used by the parser and the first argument is
the state. These two can be passed to the error function with the type signature
C<< val message : int -> token -> string >>. Further formatting can be done in
the client of the error module.

=head3 Ocamlbuild integration

Merr can be easily integrated with C<ocamlbuild>. See its own C<myocamlbuild.ml>
and C<_tags> files to understand how to do that.
